...
def read_texture(filename =r'.\scene\anvil_top_damaged_0.png'):
    try:
...
void main()
{
    ...
    Quad light;
    light.pos = vec3[4](
        vec3(-1.75 + sin(iTime), 0.0 + lightsPos.x, lightsPos.y + cos(iTime) ),
        vec3(-1.75+ sin(iTime), 0.5 + lightsPos.x, lightsPos.y+ cos(iTime)),
        vec3( 1.75 + sin(iTime), 0.5 + lightsPos.x, lightsPos.y+ cos(iTime)),
        vec3( 1.75+ sin(iTime), 0.0 + lightsPos.x, lightsPos.y+ cos(iTime))
    );
    
    light.color = vec3(1, cos(iTime), sin(iTime));
